...
/**
 * Mina解码 单个报文首次发送与接收数据的大小至少应该大于9个字节，由[1个字节通信类型（1建连接;2心跳;3业务通讯）][4个字节字符串长度][4
 * 个字节byte数组长度]这个而得出来的
...
public class MinaTransportDecoder implements MessageDecoder {
    ...
    public MessageDecoderResult decodable(IoSession session, IoBuffer in) {
        ...
        if (type != ConstantTransport.MINA_PROTOCOL_TYPE_1 && type != ConstantTransport.MINA_PROTOCOL_TYPE_2 && type != ConstantTransport.MINA_PROTOCOL_TYPE_3) {
            throw new IllegalArgumentException(TransportException.EORROR_COMMANDPROTOCOL + ", Message type is incorrect, type=" + type + ", sessionId="
                    + session.getId());
        }
        if ((binaryLenth + jsonStrLenth) > ConstantTransport.MINA_MAX_SIZE_BINARY)// 字符串与byte[]总大小大于100M
        {
            throw new IllegalArgumentException(TransportException.EORROR_COMMANDPROTOCOL + ", Message string and binary is to large. string size="
                    + jsonStrLenth + ", binary size=" + binaryLenth + ", sessionId=" + session.getId());
        }
        try {
            context = new Context(type, jsonStrLenth, binaryLenth);
        } catch (Throwable e) {
            throw new IllegalArgumentException(TransportException.EORROR_COMMANDPROTOCOL + ", Message string and binary is to large. string size="
                    + jsonStrLenth + ", binary size=" + binaryLenth + ", type=" + type + ", sessionId=" + session.getId(), e);
        }
    ...
    public MessageDecoderResult decode(IoSession session, IoBuffer in, ProtocolDecoderOutput out) throws Exception {
        ...
        if (context == null) {
            throw new TransportCommandProtocolException(TransportException.EORROR_COMMANDPROTOCOL, "IoSession's attribute not has context. session id="
                    + session.getId());
        }

        byte type = context.type;
        if (type != ConstantTransport.MINA_PROTOCOL_TYPE_1 && type != ConstantTransport.MINA_PROTOCOL_TYPE_2 && type != ConstantTransport.MINA_PROTOCOL_TYPE_3) {
            throw new TransportCommandProtocolException(TransportException.EORROR_COMMANDPROTOCOL, "Message type is incorrect, type=" + type + ", sessionId="
                    + session.getId());
        }
        if ((context.binaryLenth + context.jsonLenth) > ConstantTransport.MINA_MAX_SIZE_BINARY)// 字符串与byte[]总大小大于100M
        {
            throw new TransportCommandProtocolException(TransportException.EORROR_COMMANDPROTOCOL, "Message string and binary is to large. string size="
                    + context.jsonLenth + ", binary size=" + context.binaryLenth + ", sessionId=" + session.getId());
        }
