...
import net.sample.framework.message.common.SemaphoreOnce;
...
public abstract class MinaTransportAbstract
{
    ...
    //protected ExecutorService loginDistributedProcessorExecutorService;//为了资源，改用getAsyncProcessorExecutor()
    ...
    protected void processRequestCommandImpl(final IoSession session, final String add, final RemotingCommand request)
    {
            ...
            public void run()
            {
                try
                {
                    ...
                    if (!MinaTransportAbstract.this.loginItype.equals(itype))//非登录
                    {
                        ...
                        if (MinaTransportAbstract.this.moniterItype.equals(itype))
                        {//监控接口，更新映射数据
                            if (MinaTransportAbstract.this.isLong())
                            {//长连接才处理
                        ...
                        if (rpc == 0)//（同步应答）为0
                        {
                            ...
                            if (response == null)
                            {
                                ...
                                throw new TransportException(TransportException.EORROR_RESPONSE_SYNC_NULL, "同步应答返回为null, Request Serial=" + request.getHeader().getSerial() + ", add=" + add + ", itype=" + request.getHeader().getItype());
                            }
                        }
                        else if (rpc == 1)//（异步应答）为1
                        {
                            ...
                            {
                                String info = "Too many requests and AsyncProcessorThread pool busy,processRequestAsync RejectedExecutionException. request=" + request;
                                TransportLog.error(info, e);
                                throw new TransportTooMuchRequestException(TransportException.EORROR_TOOMUCHREQUEST, "请求大多,服务器忙. Request Serial=" + request.getHeader().getSerial() + ", add=" + add + ", itype=" + request.getHeader().getItype());
                            }
                    ...
                    {//登录
                        ...
                        {
                            TransportLog.warn("loginProcessor processLoginRequestSync exception,Request Serial=" + request.getHeader().getSerial() + ", add=" + add + ", Request=" + request + ", e=" + e);
                            throw e;
                        }
                        ...
                        {
                            TransportLog.warn("Login response is null, Request Serial=" + request.getHeader().getSerial() + ", add=" + add + ", Request=" + request);
                            throw new TransportException(TransportException.EORROR_RESPONSE_LOGIN_NULL, "无效连接. Request Serial=" + request.getHeader().getSerial() + ", add=" + add);
                        }
                        ...
                        //登录成功后把1:mac存入到addsMap中, 2：IoSession中放入 add=ClientInfoMsg
                ...
                {
                    TransportLog.warn(String.format("processRequestCommandImpl:processRequest is TransportException. request is [%s], e is [%s]", request, e.getMessage()));
                ...
                catch (Throwable e)
                {
                    ...
                    MinaTransportAbstract.this.writeErrorReqsponse(session, request, add, new TransportException(TransportException.EORROR_TRANSPORT, "processRequestCommandImpl:processRequest is Throwable. Request Serial=" + request.getHeader().getSerial() + ", add=" + add + ", itype=" + request.getHeader().getItype() + ", e=" + e.getMessage(), e));
                }
            }
        };
        ...
        catch (RejectedExecutionException e)
        {
            ...
            TransportLog.error(info, e);
            TransportException te = new TransportTooMuchRequestException(TransportException.EORROR_TOOMUCHREQUEST, "请求大多,服务器忙. Request Serial=" + request.getHeader().getSerial() + ", add=" + add + ", itype=" + request.getHeader().getItype());
    ...
    protected void processResponseCommandImpl(IoSession session, String add, final RemotingCommand response)
    {
        ...
        {
            TransportLog.info("Receive response, but not matched any request, response may be timeout, response Serial=" + response.getHeader().getSerial() + ", add=" + add + ", itype=" + response.getHeader().getItype() + ", response=" + response);
            this.isClose(session);
            return;
        }

        if (MinaTransportAbstract.this.RpcHook != null)
        {//校验报文，由于是应答，出错只记录日志即可
            try
            {
            ...
            {
                TransportLog.warn("Receive response, but RpcHook.doBeforeRequest transportCommandException. response=" + response + ", transportCommandException=" + e);
            }
        }
        ...
        if (responseFuture.getCallback() != null)
        {
            ...
            if (executor != null)
            {
                try
                {
                    executor.submit(new Runnable()
                    {
                        ...
                        public void run()
                        {
                            ...
                            {
                                TransportLog.debug("Excute InvokeCallback in CallbackExecutor exception, and excuteInvokeCallback exception, responseFuture=" + responseFuture + ", e=" + e.getMessage());
                            }
                        }
                    });
                ...
                {
                    TransportLog.debug("Excute InvokeCallback in CallbackExecutor exception, maybe CallbackExecutor is busy, responseFuture=" + responseFuture + ", e=" + e.getMessage());
                }
            }
    ...
    protected RemotingCommand invokeSyncImpl(IoSession session, final String add, final RemotingCommand request, long timeoutMillis) throws InterruptedException, TransportTimeoutException, TransportSendRequestException
    {
        ...
        try
        {
            ...
            session.write(request).addListener(new IoFutureListener<IoFuture>()
            {
                ...
                public void operationComplete(IoFuture future)
                {
                    ...
                    TransportLog.info("invokeSyncImpl send exception. serial=" + serial + ", itype=" + request.getHeader().getItype() + ", add=" + add + ", request=" + request);
                }
            });
            ...
            if (response == null)
            {
                ...
                { //发送报文成功，读取应答超时
                    throw new TransportTimeoutException(TransportException.EORROR_TIMEOUT, add, timeoutMillis, "invokeSyncImpl: serial=" + serial + ", itype=" + request.getHeader().getItype() + ", add=" + add, responseFuture.getThrowable());
                }
                else
                {//发送报文失败
                    throw new TransportSendRequestException(TransportException.EORROR_SENDREQUSET, add, "invokeSyncImpl: serial=" + serial + ", itype=" + request.getHeader().getItype() + ", add=" + add, responseFuture.getThrowable());
                }
            }
    ...
    //短连接不能主动发数据，长连接发数据是否登录等不用这这儿处理
    protected void invokeASyncImpl(IoSession session, final String add, final RemotingCommand request, long timeoutMillis, InvokeCallback invokeCallback) throws InterruptedException, TransportTimeoutException, TransportSendRequestException, TransportTooMuchRequestException
    {
        final String serial = request.getHeader().getSerial();
            ...
            try
            {
                session.write(request).addListener(new IoFutureListener<IoFuture>()
                {
                    ...
                    public void operationComplete(IoFuture future)
                    {
                        ...
                        TransportLog.info("invokeASyncImpl send exception. serial=" + serial + ", itype=" + request.getHeader().getItype() + ", add=" + add + ", request=" + request);
        ...
        else
        {
            if (timeoutMillis <= 0)
            {
                throw new TransportTooMuchRequestException(TransportException.EORROR_TOOMUCHREQUEST, "invokeASyncImpl invoke too fast, timeoutMills=" + timeoutMillis);
            }
            else
            {
                String info = String.format("invokeASyncImpl require semaphore timeout,requset serial is [%s], timeoutMills is [%d](ms), waiting thread nums is [%d], semaphoreAsync value is [%d].", serial, timeoutMillis, this.semaphoreAsync.getQueueLength(), this.semaphoreAsync.availablePermits());
    ...
    protected void invokeUnreplyImpl(IoSession session, final String add, final RemotingCommand request, long timeoutMillis) throws InterruptedException, TransportTooMuchRequestException, TransportTimeoutException, TransportSendRequestException
    {
        ...
        if (required)
        {
            ...
            try
            {
                session.write(request).addListener(new IoFutureListener<IoFuture>()
                {
                    ...
                    public void operationComplete(IoFuture future)
                    {
                        ...
                        if (!future.isDone())
                        {
                            TransportLog.warn(String.format("InvokeUnreplyImpl invoke IoFuture.isDone() is false. write send a request to [%s] failed. request is [%s]", add, request.toString()));
                        }
                        else
                        {
                            TransportLog.debug(String.format("InvokeUnreplyImpl invoke . write send a request to [%s] success. request is [%s]", add, request.toString()));
                        }
                    }
                });
            ...
            {
                sOnce.release();
                TransportLog.warn(String.format("invokeUnreplyImpl invoke. write send a request to [%s] failed. request serial is [%s]", add, request.getHeader().getSerial()));
                throw new TransportSendRequestException(TransportException.EORROR_SENDREQUSET, add, request.toString(), e);
            }
        }
        else
        {
            if (timeoutMillis <= 0)
            {
                throw new TransportTooMuchRequestException(TransportException.EORROR_TOOMUCHREQUEST, "invokeUnreplyImpl invoke too fast, timeoutMills=" + timeoutMillis);
            }
            else
            {
                String info = String.format("invokeUnreplyImpl require semaphore timeout,requset serial is [%s], timeoutMills is [%d](ms), waiting thread nums is [%d], semaphoreUnreply value is [%d].", request.getHeader().getSerial(), timeoutMillis, this.semaphoreUnreply.getQueueLength(), this.semaphoreUnreply.availablePermits());
                TransportLog.warn(info + ". request is [" + request + "].");
                throw new TransportTimeoutException(TransportException.EORROR_TIMEOUT, info);
            }
        }
    ...
    private void processDistributedlogin(final LoginDistributedProcessor l, final ClientInfoMsg clientInfoMsg, final IoSession session, final String add, final RemotingCommand request, final int type) throws TransportTooMuchRequestException
    {
        try
        {
            //MinaTransportAbstract.this.loginDistributedProcessorExecutorService.submit(new Runnable()
        ...
        {
            String info = "Too many requests and loginDistributedProcessorThread pool busy, processShareSession and processHistoryData RejectedExecutionException. request=" + request;
