...
public class HttpClientUtil {
    ...
    private final static String INTERFACE_SERIAL  = "net.sebinson.interface.serial";
    private final static String INTERFACE_CODE    = "net.sebinson.interface.code";
    private final static String SYS_CONNECT_ERROR = "S001";

    /**
     * web层通过此方法调用interface层 会同时传递Parametert和Attribute中的参数 如果Parametert和Attribute参数重名则接收端会以数组的形式全都取到
     * 
     * @param code
     * @param request
    ...
    public static <T> TransData<T> transfer(String code, String url, HttpServletRequest request, Class<T> clazz) {
        TransDataHead defHead = new TransDataHead(false, SYS_CONNECT_ERROR);
    ...
    private static Object transferObj(String code, String url, List<BasicNameValuePair> nvps) {
        ...
        try {
            ...
            nvps.add(new BasicNameValuePair(INTERFACE_SERIAL, UUIDUtil.getUUID()));
            logSend(nvps); //记日志
